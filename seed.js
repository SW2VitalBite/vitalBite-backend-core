const { PrismaClient } = require('./src/prisma/generated-client/index.js');
const { Pool } = require('pg');
const { PrismaPg } = require('@prisma/adapter-pg');

const connectionString = 'postgresql://postgres:postgres@localhost:5432/vitalbite_core';
...
async function main() {
  ...
  const nutritionist = await prisma.user.findFirst({
    where: { roleCode: { in: ['NUTRICIONISTA', 'ADMINISTRADOR', 'ADMIN'] } },
  });

  if (!tenant || !nutritionist) {
    console.error('No se encontró un Tenant o Nutricionista base para insertar pacientes.');
    return;
  }
  ...
  for (const [i, mock] of mockPatients.entries()) {
    const patient = await prisma.patient.create({
      data: {
        ...
        email: `paciente${i + 1}@vitalbite.local`,
  ...
  console.log(`✅ ¡Se han insertado ${count} pacientes de prueba exitosamente!`);
}
